...
function aff_cart_redirect() {
	$role = get_user_info( 'role' );
	if ( $role=='affiliate' ) {
		header( 'Location: http://thecannabismethod.com/' );
		exit;
	}
}
...
function add_specials_menu() {
	...
	if ( $level=='Platinum' || $role=='admin') {
		...
		<script>
			jQuery(document).ready(function($) {
				$('#menu-item-17 > ul > li:first-of-type').before('<li class=\"menu-item menu-item-type-custom menu-item-object-custom\"><a href=\"http://thecannabismethod.com/?sort=menu_order&spl=1\"><b>10 Gram Specials</b></a></li>');
			});
...
function wc_minimum_order_amount() {
	...
	if (strstr(' 90024 90027 90028 90029 90034 90036 90038 90039 90045 90046 90048 90049 90064 90066 90067 90068 90069 90095 90230 90291 90401 90402 90403 90404 90405 91505 91506 91602 91604 91607 91608 ', $postcode))
		$minimum = 60;
	else if	(strstr(' 90005 ', $postcode))
		$minimum = 70;
	else if	(strstr(' 90004 ', $postcode))
		$minimum = 75;
	else if	(strstr(' 90016 90026 90035 90056 90211 90212 90292 91423 ', $postcode))
		$minimum = 80;
	else if	(strstr(' 90008 90018 90045 90049 90210 90245 90272 90293 90293 ', $postcode))
		$minimum = 100;
	else if	(strstr(' 90007 90015 90017 90057 90071 90089 90266 ', $postcode))
		$minimum = 120;
	else if	(strstr(' 90013 90014 ', $postcode))
		$minimum = 150;
	else
		$minimum = 60;
    ...
    if ( WC()->cart->subtotal < $minimum ) {
        ...
        if( is_cart() ) {
            ...
            wc_print_notice(
                sprintf( 'Based on your zip code '.$postcode.', you must have an order with a minimum of %s to place your order. Your current order total is %s.' , 
                    wc_price( $minimum ), 
...
function admin_sms_alert($order_id) {
	...
	$product_list = implode( ', ', $product_meta );
	
	$user_id = $order->user_id;
	$medallion_level	= get_user_meta( $user_id, 'loyalty_status', true );
	
	$admin_sms =
		"Order #{$order_id} is pending! $billing_firstname $billing_lastname purchased $product_list for \${$order_amount} using a $medallion_level discount. ".
		"Please deliver to $shipping_address1".($shipping_address2?" {$shipping_address2}":'')." or call $billing_phone.";
...
add_action("woocommerce_thankyou", "admin_sms_alert" );
...
/*function custom_login(){
	...
	if( 'wp-login.php' == $pagenow ) {
		wp_redirect('http://thecannabismethod.com/');
		exit();
	}
}
