<?php

/*
Template Name: Dashboard 2
*/

get_header();

$countdown = get_reset_date();
$points = get_user_info('loyalty_points');
$level = get_user_info('loyalty_status');
$badge = array();

if ( $level=='Green') $badge[0]='-3';
else $badge[0]='';

if ( $level == 'Silver') $badge[1]='-3';
elseif ( $points < 1500 ) $badge[1]='-2';
else $badge[1]='';

if ( $level == 'Gold') $badge[2]='-3';
elseif ( $points < 3000 ) $badge[2]='-2';
else $badge[2]='';

if ( $level == 'Platinum') $badge[3]='-3';
elseif ( $points < 6000 ) $badge[3]='-2';
else $badge[3]='';

?>

<link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/font-awesome/4.4.0/css/font-awesome.min.css">
<link rel="stylesheet" href="https://thecannabismethod.com/wp-content/themes/zerif-lite-child/css/dashboard.css">
</header>


	<div id="content" class="site-content">

<div id="dashboard" class="container">

	<!-- page header -->
	<div class="row">
		<div class="col-sm-12">
	        <h1 class="page-header"><span>Welcome back,</span> <?=get_first_name()?>!</h1>
	    </div>
	</div>
	<!-- /page header -->
	
	<!-- stats -->
	<div class="row">
		
		<div class="col-sm-4">
			<a href="../referral-panel">
				<div class="panel panel-default">
					<div class="panel-heading">
						<div class="row">
							<div class="col-xs-3">
								<img src="data:image/png;base64,"/>
							</div>
							<div class="col-xs-9 text-right">
								<div class="huge">$<?=get_credit_balance()?></div>
								<div>TCM Credits</div>
							</div>
						</div>
					</div>
					<div class="panel-body">
						<span class="pull-left">Earn Credits</span>
						<span class="pull-right"><i class="fa fa-arrow-circle-right"></i></span>
						<div class="clearfix"></div>
					</div>
				</div>
			</a>
		</div>
				
		<div class="col-sm-4">
			<a href="../referral-history">
				<div class="panel panel-default">
					<div class="panel-heading">		
						<div class="row">
							<div class="col-xs-3">
								<img src="data:image/png;base64,"/>
							</div>
							<div class="col-xs-9 text-right">
								<div class="huge"><?=count_referrals()?></div>
								<div>Total Referrals</div>
							</div>
						</div>
					</div>
					<div class="panel-body">
						<span class="pull-left">View History</span>
						<span class="pull-right"><i class="fa fa-arrow-circle-right"></i></span>
						<div class="clearfix"></div>
					</div>
				</div>
			</a>
		</div>
		
		<div class="col-sm-4">
			<?php $last_order = get_last_order(); ?>
			<a href="<?=($last_order?'../purchase-history':'../shop')?>">
				<div class="panel panel-default">
					<div class="panel-heading">
						<div class="row">
							<div class="col-xs-3">
								<img src="data:image/png;base64,"/>
							</div>
							<div class="col-xs-9 text-right">
								<?php
									if ($last_order) {
										echo "<div class='huge'>#$last_order</div>";
										echo "<div>Last Order</div>";
									} else {
										echo "<div style='font-size:28px'>ORDER<br>NOW</div>";
									}
								?>
							</div>
						</div>
					</div>
					<div class="panel-body">
						<span class="pull-left"><?=($last_order?'View History':'View Medicine')?></span>
						<span class="pull-right"><i class="fa fa-arrow-circle-right"></i></span>
						<div class="clearfix"></div>
					</div>
				</div>
			</a>
		</div>
		
	</div>
	<!-- /stats-->
	
	<!-- level -->
	<div class="row">
		<div class="col-sm-12">
			<div class="panel panel-default">
				<div class="panel-heading"><h3>Medallion Level</h3></div>
				<div class="panel-body">
					<h2><?=get_user_info('loyalty_status')?></h2>
					<div class="badge-wrap">
						<a data-toggle="modal" data-target="#myModal">
							<img class="badge-icon" src="https://thecannabismethod.com/wp-content/themes/zerif-lite-child/img/green<?=$badge[0]?>.png"
								data-toggle="tooltip" data-html="true" data-placement="top" title="Green">
							<img class="badge-icon" src="https://thecannabismethod.com/wp-content/themes/zerif-lite-child/img/silver<?=$badge[1]?>.png"
								data-toggle="tooltip" data-html="true" data-placement="top" title="Silver: 1,500 LP">
							<img class="badge-icon" src="https://thecannabismethod.com/wp-content/themes/zerif-lite-child/img/gold<?=$badge[2]?>.png"
								data-toggle="tooltip" data-html="true" data-placement="top" title="Gold: 3,000 LP">
							<img class="badge-icon" src="https://thecannabismethod.com/wp-content/themes/zerif-lite-child/img/platinum<?=$badge[3]?>.png"
								data-toggle="tooltip" data-html="true" data-placement="top" title="Platinum: 6,000 LP">
						</a>
					</div>
					<h5><a data-toggle="modal" data-target="#myModal">Medallion Benefits <i class="fa fa-arrow-circle-right" style="padding-left:10px"></i></a></h5>
					<br>
					<h2><?=get_user_info('loyalty_points')?> MP</h2>
					<div class="progress">
						<div class="progress-bar progress-bar-success" role="progressbar" aria-valuenow="<?=get_progress()?>" aria-valuemin="0" aria-valuemax="100" style="width: <?=get_progress()?>%"><span class="sr-only"><?=get_progress()?>% Complete (success)</span></div>
					</div>
					<h5><?=get_progress_counter()?></h5>
					<?=($countdown?"$countdown":'')?>
				</div>
			</div>
		</div>
	</div>
	<!-- /points -->
	
	<!-- modal -->
	<div class="modal fade" id="myModal" role="dialog">
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal">&times;</button>
					<h1 class="modal-title">TCM Medallion Benefits</h1>
				</div>
				<div id="benefits" class="modal-body text-left">
					<h3>It's Very Rewarding</h3>
					<p>The Cannabis Method rewards returning patients through our loyalty program - TCM Medallion.  When requesting a delivery from our Collective, patients earn 10 points for every $1 that is donated.  As patients accumulate more points, they unlock higher Medallion Levels that provide additional gifts and benefits.</p>
					<p>Once a Medallion level is unlocked, a patient will have 30 days to enjoy the benefits of that Medallion Level.  If a patient reaches a higher Medallion Level during that time, their benefits at the new higher level are extended for an additional 30 days.  Medallion Levels reset every 30 days.</p>
					<h3>Medallion Benefit Breakdown</h3>
					<p>The chart below summarizes the different benefits of each Medallion Level.</p>
					<div class="scroll">
						<table class="table table-bordered table-striped">
							<tbody>
								<tr>
									<td>Medallion Level</td>
									<td>Green</td>
									<td>Silver</td>
									<td>Gold</td>
									<td>Platinum</td>
								</tr>
								<tr>
									<td>Points required to unlock</td>
									<td></td>
									<td>1,500</td>
									<td>3,000</td>
									<td>6,000</td>
								</tr>
								<tr>
									<td>First time patient discount applied to your first order</td>
									<td><i class="fa fa-check"></i></td>
									<td></td>
									<td></td>
									<td></td>
								</tr>
								<tr>
									<td>Get access to premium specials & promotions</td>
									<td><i class="fa fa-check"></i></td>
									<td><i class="fa fa-check"></i></td>
									<td><i class="fa fa-check"></i></td>
									<td><i class="fa fa-check"></i></td>
								</tr>
								<tr>
									<td>Be the first know to about new product arrivals</td>
									<td><i class="fa fa-check"></i></td>
									<td><i class="fa fa-check"></i></td>
									<td><i class="fa fa-check"></i></td>
									<td><i class="fa fa-check"></i></td>
								</tr>
								<tr>
									<td>Discounted cart total with every purchase</td>
									<td></td>
									<td>$5 OFF</td>
									<td>$10 OFF</td>
									<td>$20 OFF</td>
								</tr>
								<tr>
									<td>Complimentary pre-roll with every purchase</td>
									<td></td>
									<td>single</td>
									<td>3-pack</td>
									<td>3-pack</td>
								</tr>
								<tr>
									<td>Complimentary gram with every purchase</td>
									<td></td>
									<td></td>
									<td></td>
									<td><i class="fa fa-check"></i></td>
								</tr>
								<tr>
									<td>10 Gram Specials</td>
									<td></td>
									<td></td>
									<td></td>
									<td><i class="fa fa-check"></i></td>
								</tr>
							</tbody>
						</table>
					</div>
				</div>
			</div>
		</div>
	</div>
	<!-- /modal -->

</div>

<script>
	jQuery(document).ready(function($) {
		$('[data-toggle="tooltip"]').tooltip(); 
	});
</script>


<?php get_footer();?>